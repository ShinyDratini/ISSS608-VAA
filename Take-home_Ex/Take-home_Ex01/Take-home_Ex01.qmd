...
The Gulf of Guinea is a region rich in natural resources. It holds significant reserves of gas and oil as well as other valuable minerals. It is an important shipping zone transporting oil, gas and other commodities to and from central and southern Africa. It is also a hotspot for piracy. In 2020, there were 78 piracy incidents and 130 seafarers kidnapped for ransom.

## Task

This study is conducted to find the correlation between the type of ships and and the revenue they bring in in the Gulf of Guinea. This will allow government officials to better strategize on how to improve ship performance in the region.
...
bp <- plot_ly(
  data = df,
  x = ~Efficiency_nm_per_kWh,  # ✅ Efficiency instead of Cost
  y = ~Profit,  # ✅ Profitability instead of Revenue
  size = ~Cargo_Weight_tons,  # ✅ Bubble size based on cargo weight
  color = ~Ship_Type,  # ✅ Color-coded by Ship Type
  frame = ~Date,  
  text = ~paste("Ship Type:", Ship_Type,
                "<br>Route Type:", Route_Type,
                "<br>Engine Type:", Engine_Type,
                "<br>Efficiency:", round(Efficiency_nm_per_kWh, 2),
                "<br>Profit (USD):", scales::comma(Profit)),
  hoverinfo = "text",
  type = 'scatter', 
  mode = 'markers',
  ...
  layout(
    title = "Ship Efficiency vs Profitability",
    xaxis = list(title = "Efficiency (nm per kWh)"),
...
summary_data <- df %>%
  ...
  summarise(
    ...
    Total_Revenue = sum(Revenue_per_Voyage_USD, na.rm = TRUE),
    Profit = sum(Revenue_per_Voyage_USD, na.rm = TRUE) - sum(Operational_Cost_USD, na.rm = TRUE),
    Profit_Margin_Percentage = (Profit / Total_Revenue) * 100,  # ✅ Compute Profit Margin
    Average_Load_Percentage = mean(Average_Load_Percentage, na.rm = TRUE),  # ✅ Aggregate properly
    Turnaround_Time = mean(Turnaround_Time_hours, na.rm = TRUE),
    .groups = "drop"
  )
...
bp <- plot_ly(
  data = summary_data,
  x = ~Average_Load_Percentage,  # ✅ How full the ship is
  y = ~Profit_Margin_Percentage,  # ✅ How much profit per revenue
  size = ~Turnaround_Time,  # ✅ Bubble size based on turnaround time
  color = ~Route_Type,  # ✅ Color-coded by Route Type
  frame = ~Date,  
  text = ~paste("Ship Type:", Ship_Type,
                "<br>Route Type:", Route_Type,
                "<br>Load Factor (%):", Average_Load_Percentage,
                "<br>Profit Margin (%):", round(Profit_Margin_Percentage, 2),
                "<br>Turnaround Time (days):", Turnaround_Time),
  hoverinfo = "text",
  type = 'scatter', 
  mode = 'markers',
  marker = list(line = list(width = 0))
  ...
  layout(
    title = "Load Factor vs Profit Margin",
    xaxis = list(title = "Average Load Percentage (%)"),
